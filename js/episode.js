...
right.addEventListener('click', () => {
    i = (i + 1) % Math.ceil(reversedEpisodes.length / cards.length); // índice de página

    Array.from(cards).forEach((card, index) => {
        const episodeIndex = i * cards.length + index;
        const episode = reversedEpisodes[episodeIndex];
        if (!episode) {
            card.style.display = "none"; // Oculta si no hay episodio
            return;
        }
    ...
    numbers[numberValue].style.color = "";
    numberValue++;
    console.log;
    numbersIndex();
});
